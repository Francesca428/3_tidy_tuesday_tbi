...
here_tbi_age <- read_csv(here("raw_data","https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-03-24/tbi_age.csv"))
```
...
#There are 5 variables: age_group, type, injury mechanism, number_est, rate_est. The first 3 are characters and the other 2 are numeric. There are 231 rows of data/people in the dataset.
...
```{r most-deaths}
tbi_age_deaths <- tbi_age %>%
  filter(type == "Deaths")

desc_tbi_age_deaths <- tbi_age_deaths %>%
  arrange(injury_mechanism, desc(number_est))
desc_tbi_age_deaths
```
...
tbi_age_totals <- tbi_age %>%
  arrange(injury_mechanism, injury_mechanism)
tbi_age_totals

tbi_totals <- tbi_age_totals %>%
  select(type, injury_mechanism, number_est)
tbi_totals

tbi_test <- tbi_age_totals %>%
  filter(c(injury_mechanism == "Assault", type == "Emergency Department Visit"))
